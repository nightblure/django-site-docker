...
from newsapp.tests.utils import send_authenticated_request, send_no_auth_request, get_url
...
def test_create_already_posted_news(admin_user):
    ...
    response = send_authenticated_request(
        get_url('news_create_api_route'),
        admin_user, NewsCreateApi,
        (), 'post', body
    )
...
def test_delete_news_forbidden(no_admin_user):
    ...
    response = send_authenticated_request(
        url, no_admin_user, NewsDeleteApi,
        (news.slug,), 'delete'
    )
...
def test_update_news_forbidden(author, author_or_other_user, expected_code, request):
    ...
    response = send_authenticated_request(
        url, author_or_other_user_, NewsUpdateApi,
        (news.slug,), 'patch'
    )
...
def test_update_news_api_success(admin_user):
    ...
    random_news = mixer.blend(
        News, title=mixer.RANDOM, content=mixer.RANDOM,
    ...
    response = send_authenticated_request(
        url, admin_user, NewsUpdateApi,
        (news.slug,), 'patch', serializer.data
    )
